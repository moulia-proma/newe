  ...
  private  var stdId =""
    ...
    private fun initializeFlowCollectors() {
      ...
      /*                      val totalPos = data.data?.let { viewModel.getTotalPosMarks(it) }
                            ...
                            val negAchived = data.data?.let { viewModel.getTotalAchivedNegMarks(it) }

                            binding.textPositiveMark.text = "Positive ${posAchived}/${totalPos}"
                            binding.textNegativeMark.text = "Negative -${negAchived}/-${totalNeg}"
